...
// GUI 記述のための ScalaFX ライブラリ (http://www.scalafx.org/docs/quickstart/)

val scala_fx = Seq("org.scalafx" %% "scalafx" % "8.0.144-R12") ++
  "base,controls,fxml,graphics,media,swing,web".split(",").map(m =>
    "org.openjfx" % s"javafx-$m" % "11" classifier osName).toSeq
...
lazy val common_fx = common ++ Seq(libraryDependencies ++= scala_fx)
...
lazy val lx03  = project.settings(common)
...
lazy val lx08 = (project in file("lx08")).settings(common)
lazy val lx09 = (project in file("lx09")).settings(common)
lazy val lx10 = (project in file("lx10")).settings(common)
